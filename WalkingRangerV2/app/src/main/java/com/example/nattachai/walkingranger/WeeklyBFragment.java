...
public class WeeklyBFragment extends ListFragment {
  ...
  //  ArrayList<String> leads = new ArrayList<String>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        new SimpleTask().execute("http://203.151.92.179:8080/getgroupsortweekly?id=" + MemberStatic.getFbID());
    ...
    private void showData(String jsonString) {
        ...
        for(int i = 0; i < groups.size(); i++)
        {
            ...
            int k=String.valueOf(groups.get(i).getAvgWeeklySteps()).length();
            for(int j=k;j<30;j++){
                leads[i]+="  ";
            }
            leads[i]+=groups.get(i).getAvgWeeklySteps()+"       ";
        }
